...
function waterFall(boxid='list_ul',gap=22,itemWidth=310) {
...
function input_size() {
    ...
    if(swidth==0 && sheight==0){
        g('ms').innerHTML='dimensión mínima';
    }else{
...
if(g('loader'))
{
    g('loader').addEventListener('click',function () {
        g('loader').disabled=true;
        var bd='page='+g('loader').value;
        if(location.href.indexOf('?lang=')>0)
        {
            bd='lang='+location.href.split('?lang=').pop()+'&'+bd;
        }
        nanoajax.ajax({url:g('loader').dataset.url, method: 'POST', body: bd}, function (code, html) {
    ...
    window.onscroll= function  () {
        ...
        if (srcollTop+windowHeight  >= srcollH-h) {
            g('loader') && g('loader').click();
        };
    }
}
...
function download(token){
    ...
    var savename='pngocean.com.png';
    nanoajax.ajax({url:site+'/verificar', method: 'POST', body: 'id='+id+'&token='+token}, function (code, res) {
        var obj = JSON.parse(res);
        if(obj.status){
            g('download_secs').innerHTML=l;
            if(location.href.indexOf('?ancho=')>0 || location.href.indexOf('?altura=')>0){
                var img=new Image();
                ...
                img.onload=function () {
                    ...
                    if(location.href.indexOf('?ancho=')>0){
                        var tw=parseInt(location.href.split('?ancho=').pop());
                        var th=tw * img.naturalHeight / img.naturalWidth;
                    }else{
                        var th=parseInt(location.href.split('?altura=').pop());
            ...
            g('download_secs').innerHTML='¡Solicitud fallida! por favor cambia tu navegador o red';
        }
    })
}
